        ...
        {selected ? (
          <>
            ...
            <section className="release-panel">
              ...
              {validation && <div className={"validation-result validation-" + validation.status}><strong>{validation.status.toUpperCase()}</strong><span>Context {validation.context.contextHash.slice(0, 12)}</span><span>Ghost Replay: {validation.ghostJournal.length} event(s), non-authoritative</span>{validation.error && <span>Runtime failure: {validation.error}</span>}{validation.reviewAcknowledgement && <span>Reviewed by {validation.reviewAcknowledgement.actor}: {validation.reviewAcknowledgement.reason}</span>}{validation.differentialDeletions.length > 0 && <span>Differential block: new deletions — {validation.differentialDeletions.join(", ")}</span>}{validation.status === "baseline_unhealthy" && <span>The active release failed its own gates on this task — the candidate was not judged against it.</span>}{validation.status === "review_required" && <span>Human acknowledgement recorded; promotion requires an audited actor and reason.</span>}{validation.baselineGateFailures.map((failure, index) => <span key={"b" + failure.code + index}>Active release gate {failure.code}: {failure.reason}</span>)}{validation.candidateGateFailures.map((failure, index) => <span key={"c" + failure.code + index}>Candidate gate {failure.code}: {failure.reason}</span>)}{validation.candidateDiff.changes.length > 0 && <span>Candidate diff: {validation.candidateDiff.changes.map((change) => change.kind + " " + change.path).join(", ")}</span>}</div>}
            </section>
