...
export interface ValidationRecord { id: string; status: "running" | "certified" | "blocked" | "baseline_unhealthy" | "failed"; task: string; candidateReleaseId: string; baselineDiff: { changes: FileChange[] }; candidateDiff: { changes: FileChange[] }; baselineGateFailures: { code: string; reason: string }[]; candidateGateFailures: { code: string; reason: string }[]; differentialDeletions: string[]; context: { contextHash: string }; error: string | null; }
