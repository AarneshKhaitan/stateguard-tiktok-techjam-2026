...
export interface ValidationRecord { id: string; status: "running" | "certified" | "blocked" | "baseline_unhealthy" | "review_required" | "failed"; task: string; candidateReleaseId: string; baselineDiff: { changes: FileChange[] }; candidateDiff: { changes: FileChange[] }; baselineGateFailures: { code: string; reason: string }[]; candidateGateFailures: { code: string; reason: string }[]; differentialDeletions: string[]; novelEffects: string[]; historyRecordCount: number; historyMinRecords: number; bisection: { culpritSegments: string[]; inconclusive: boolean; probes: { reproduced: boolean }[] } | null; context: { contextHash: string }; error: string | null; reviewAcknowledgement: { actor: string; reason: string; acknowledgedAt: string } | null; promotionAudit: { actor: string; reason: string; promotedAt: string } | null; ghostJournal: { sequence: number; kind: string; path: string; contentBase64?: string }[]; }
